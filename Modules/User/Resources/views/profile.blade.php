		...
		<div class="col-lg-12">
            {{ Form::open(array('method' => 'get' , 'url' => route('route.user.update' , ['id' => $user->id]) )) }}
    			<div class="form-group">
                    <label>Email</label>
                    <input class="form-control"  {{ $isAdmin ? 'disabled' : '' }} name="email" value="{{ $user->email }}" placeholder="Email">
                </div>
                <div class="form-group">
                ...
                <button type="submit" name="update_user" class="btn btn-default">Update</button>
		  {{ Form::close() }}
        </div>
	</div>
